		...
		<div className="container p-5 pt-0 mx-auto">
				...
				<div className="row d-flex justify-content-center mt-4 px-2">
					{store.user.id != task.requester_user?.id 
						? ((store.user.role == "both" || store.user.role == "task_seeker") && !applied) && <button className="btn btn-green px-4" onClick={handleShowApplicationForm}>Apply</button>
						: task.status == "pending" && <button className="btn btn-green px-4" onClick={handleShowTaskForm}><Icon icon="mage:edit-fill" /> Edit</button>
					}
				</div>
